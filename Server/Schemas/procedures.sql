...
BEGIN
    ...
    SELECT count(*) into is_upvoted from voters where user_id = v_user_id AND post_id = v_post_id AND vote_type = 'l';
    SELECT count(*) into is_downvoted from voters where user_id = v_user_id AND post_id = v_post_id AND vote_type = 'd';
    DELETE from voters WHERE user_id = v_user_id AND post_id = v_post_id;
    ...
    ELSEIF is_downvoted = 1 THEN
        ...
        INSERT INTO voters VALUES(v_user_id, v_post_id, 'l');
    ELSE
    	UPDATE posts SET likes = likes + 1 WHERE post_id = v_post_id;
        INSERT INTO voters VALUES(v_user_id, v_post_id, 'l');
    END IF;
...
BEGIN
    ...
    IF is_upvoted = 1 THEN
        ...
        INSERT INTO voters VALUES(v_user_id, v_post_id, 'd');
    ELSEIF is_downvoted = 1 THEN
